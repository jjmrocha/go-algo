// Package unionfind implements a union-find (disjoint-set) data structure using
// weighted quick-union with path compression for efficiently tracking a
// collection of elements partitioned into non-overlapping sets.
package unionfind
...
// UnionFind is a disjoint-set data structure that tracks a
...
func (u *UnionFind) Union(p, q int) error {
	rp, err := u.Find(p)
	if err != nil {
		return err
	}
...
func (u *UnionFind) Connected(p, q int) (bool, error) {
	rp, err := u.Find(p)
	if err != nil {
		return false, err
	}
