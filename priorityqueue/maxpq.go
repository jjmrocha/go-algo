...
// MaxPQ is a maximum priority queue implementation using a binary heap
type MaxPQ[T constraints.Ordered] struct {
	pq []T
	n  int
}

// NewMaxPQ creates a new maximum priority queue
func NewMaxPQ[T constraints.Ordered]() *MaxPQ[T] {
	return &MaxPQ[T]{
